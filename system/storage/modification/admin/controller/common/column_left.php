...
class ControllerCommonColumnLeft extends Controller {
	public function index() {
		if (isset($this->request->get['token']) && isset($this->session->data['token']) && ($this->request->get['token'] == $this->session->data['token'])) {
			...
			if ($this->user->hasPermission('access', 'megamenu/megamenu')) {
				$data['menus'][] = array(
					'id'       => 'megamenu',
					'icon'	   => 'fa-bars', 
					'name'	   => 'Mega Menu',
					'href'     => $this->url->link('megamenu/megamenu', 'token=' . $this->session->data['token'], true),
					'children' => '',
				);	
			}
	...
	if ($this->user->hasPermission('access', 'review/store_review')) {
			...
			*/	
			...
			if ($this->user->hasPermission('access', 'tltslideshow/tltslideshow')) {
				$design[] = array(
					'name'     => $this->language->get('text_tltslideshow'),
