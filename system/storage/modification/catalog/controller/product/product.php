...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			'text' => 'Каталог',
			'href' => $this->url->link('extension/module/brainyfilter/filter')
		);
		...
		if ($product_info) {
			...
			$data['composition'] = html_entity_decode($product_info['composition'], ENT_QUOTES, 'UTF-8');
			$data['lifemattress'] = $product_info['lifemattress'];
			$data['kolvolifes'] = $product_info['kolvolifes'];
			$data['linksale'] = $product_info['linksale'];
			...
			if ((float)$product_info['special']) {
				$data['specialprice'] = $this->currency->format($this->tax->calculate($product_info['price'] - $product_info['special'], $product_info['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']);
			} else {
				$data['specialprice'] = false;
			}
			...
			if ((float)$product_info['special']) {
				$data['sale'] = '-' . round(100 - ($product_info['special']*100/$product_info['price'])) . '%';
			} else {
				$data['sale'] = false;
			}
			...
			foreach ($results as $result) {
				...
				if ((float)$result['special']) {
					$sale = '-' . round(100 - ($result['special']*100/$result['price'])) . '%';
				} else {
					$sale = false;
				}
